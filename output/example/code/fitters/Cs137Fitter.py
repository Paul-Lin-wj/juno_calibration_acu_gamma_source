...
def Cs137_calculate_components(dict_result, bins_center, dict_bkg, bins_fit):
...
class Cs137Fitter:
    def __init__(
        ...
        bkg_path: str = f"{PROJECT_ROOT}/Cs137_Compton_BKG.npz",
        x_limit: float = 0.3,
        if_fix_abc: bool = True,
        enable_c14: bool = True,
        fix_c14_amplitude: bool = False,
    ):
        self.MC_Qedep_Center = 0.58423
    ...
    def __call__(
        ...
        # print(f"Energy resolution: {energy_res}")
    ...
    def _setup_minuit(self):
        ...
        print(len(self.bkg_data["Compton"]))
        ...
        m = Minuit(
            ...
            Compton=np.max(self.data_binned[self.index_nonzero]) * 2,
        ...
        print(f"Compton max value: {np.max(self.data_binned[self.index_nonzero]) * 10}")
    ...
    def _organize_component(self):
        ...
        # 计算所有成分
        components = Cs137_calculate_components(
...
def Cs137FitterRunner(
    ...
    if_show_ylog=True,
    xlimit: float = 0.3,
    bkg_path: str = "Cs137_Compton_BKG.npz",
):
    """运行Cs137拟合器"""
    energy_fitter = Cs137Fitter(
        bins_fit=bins_fit,
...
def Cs137_plot_results_with_residuals(
    EnergyFitclss: object,
    title_latex: str,
    fig_path: str,
    ylabel_show: str,
